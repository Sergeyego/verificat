...
QVariant ModelRep::data(const QModelIndex &item, int role) const
{
    if (role==Qt::BackgroundRole){
        QDate dend=this->data(this->index(item.row(),4),Qt::EditRole).toDate();
...
{
    QSqlQuery query;
    query.prepare("select vd.nam, vd.numb, vd.ver_per, pv.dat as dat_beg, (pv.dat + interval '1 month'*vd.ver_per)::date as dat_end "
                  "from ver_dev vd "
    ...
    if (execQuery(query)){
        ...
        this->setHeaderData(1,Qt::Horizontal,tr("Серийный номер"));
        this->setHeaderData(2,Qt::Horizontal,tr("Период поверки, мес"));
        this->setHeaderData(3,Qt::Horizontal,tr("Дата последней поверки"));
        this->setHeaderData(4,Qt::Horizontal,tr("Дата следующей поверки"));
    }
}
